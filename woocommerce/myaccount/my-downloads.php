		...
		<?php foreach ( $downloads as $download ) : ?>
			<li>
				...
				if ( is_numeric( $download['downloads_remaining'] ) ) {
					...
					$my_downloads_count = $download['downloads_remaining'];
					echo apply_filters( 'woocommerce_available_download_count', '<span class="woocommerce-Count count">' . sprintf( _n( '%s download remaining', '%s downloads remaining', $my_downloads_count, 'woocommerce' ), $download['downloads_remaining'] ) . '</span> ', $download ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
